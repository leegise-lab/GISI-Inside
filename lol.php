...
$query = "select * from lol order by date desc limit $limit, $list";
$result = mysqli_query($connect, $query);

?>
<h2 align=center style="margin-top: 30px">리그오브레전드 갤러리</h2>
<table align = center>
    <thead align = "center">
    <tr>
...
<div class = text>
    <button style="cursor: hand"onClick="location.href='lolwrite.php'">글쓰기</button>
</div>
<div class="paging" style="margin-left: 49%;margin-top: 50px">
    ...
    <?}else{?>
        <font size=2 ><a href="lol.php?page=&list=<?=$list?>">처음</a></font>
    <?}
    if($block <=1){?>
        <font> </font>
    <?}else{?>
        <font size=2><a href="lol.php?page=<?=$b_start_page-1?>&list=<?=$list?>">이전</a></font>
    ...
    for($j = $b_start_page; $j <=$b_end_page; $j++)
    {
        ...
        <?}else{?>
            <font size=2><a href="lol.php?page=<?=$j?>&list=<?=$list?>"><?=$j?></a></font>
        <?}
    }
    ...
    if($block >= $total_block){?>
        <font> </font>
    <?}else{?>
        <font size=2><a href="lol.php?page=<?=$b_end_page+1?>&list=<?=$list?>">다음</a></font>
    <?}
    if($pageNum >= $total_page){?>
        <font size=2  color=red>마지막</font>
    <?}else{?>
        <font size=2><a href="lol.php?page=<?=$total_page?>&list=<?=$list?>">마지막</a></font>
    <?}
