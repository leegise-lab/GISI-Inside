...
if ($select == "title") {
    $query = "select * from baseball where title LIKE '%$search%' order by no desc limit $limit, $list";
} else if ($select == "contents") {
    $query = "select * from baseball where contents LIKE '%$search%' order by no desc limit $limit, $list";
} else if ($select == "id") {
    $query = "select * from baseball where id LIKE '%$search%'order by no desc limit $limit, $list";
} else {
    $query = "select * from baseball order by no desc limit $limit, $list";
}
...
<table align = center>
    ...
    while($rows = mysqli_fetch_assoc($result)) {
        ...
        $getvideo = $rows['video'];
        $reply = "select * from baseballreply where time = '$time' and isdeleted = 0";
        $replyresult = mysqli_query($connect, $reply);
        $replycnt = mysqli_num_rows($replyresult);
        ?>
<!--        게시글 클릭시 이동하는 부-->
        <form method="get" action="baseballpostview.php">
            <td width="50" align="center"><?php echo $rows['no'] ?></td>
            <td class="target" width="500" align="left" id="title">
                <a class="title" href="baseballpostview.php?page=<?php echo $pageNum?>&no=<?php echo $rows['no'] ?>">

<!--                    이미지나 동영상이 있으면 yesimg.png부분, 없으면 noimg.png-->

                    <?php if (strpos($getcontents, "<img src=\"/smarteditor2/upload/") || $getvideo != "") {
                ...
                <a name="replycount" style="color: black; font-size: 11px;"
                   href="baseballpostview.php?page=<?php echo $pageNum?>&no=<?php echo $rows['no'] ?>">
...
<div class = text>
    <button style="cursor: hand;width: 70px; height: 30px; float: right; margin-right: 27%; margin-top: 10px; background-color:#8C8C8C; border: none; color: #353535 "onClick="location.href='baseballwrite.php'">글쓰기</button>
</div>
...
<div class="search" style="margin-left: 42%;margin-top: 30px">
    <div class="search-container" style="float:left;">
        <form method="get" action="baseball.php" style="float:right; margin-left: 5px;margin-top: 2px" name="frm" id="frm"">
            <select id="selectbx" name="selectbx" style="background-color: white; width: 100px; height: 30px; margin-top: 2px">
                <option <?php if ($_GET['selectbx']=="title") {?> selected="selected "<?php } ?> value="title">제목</option>
                <option <?php if ($_GET['selectbx']=="contents") {?> selected="selected "<?php } ?> value="contents">내용</option>
                <option <?php if ($_GET['selectbx']=="id") {?> selected="selected "<?php } ?> value="id">작성자</option>
